...
class CustomAugmentations:
    ...
    def __call__(self, x):
        ...
        if x.ndim == 2:  # (H, W) -> (1, H, W)
            x = x.unsqueeze(0)
        elif x.ndim == 4:  # (B, C, H, W) -> 增广通常在 DataLoader 批处理前执行，去掉 B 维
            x = x.squeeze(0)
...
class SimSiam(nn.Module):
    def __init__(self, feature_dim=512):
        ...
        self.projector = MLPHead(1024, 512, feature_dim)
        self.predictor = MLPHead(feature_dim, 256, feature_dim)

    def forward(self, x1, x2):
        f1 = self.encoder(x1).view(x1.size(0), -1)
        f2 = self.encoder(x2).view(x2.size(0), -1)
...
class SiameseNetwork(nn.Module):
    ...
    def forward_once(self, x):
        return self.fc1(self.encoder(x).view(x.size(0), -1))
